...
def decorate_object_class(c, to_pdf, ignore_fields):
    name = c.__name__
    report = {
        'name': name,
        'type': type(c),
        'doc': inspect.cleandoc(c.__doc__ or 'None'),
    ...
    if to_pdf:
        report_pdf(f'{name} {type(c).__name__} Object Report', report, f'{name}_object.pdf')
    else:
        print(report)
...
def decorate_object(f, to_pdf, ignore_fields):
    if inspect.isclass(f):
        return decorate_object_class(f, to_pdf, ignore_fields)
    ...
    def ff(*args, **kwargs):
        ...
        if to_pdf:
            report_pdf(f'{name} {type(f).__name__} Object Report', report, f'{name}_object.pdf')
        else:
            print(report)
...
def decorate_complexity(f, to_pdf, ignore_fields):
    ...
    name = f.__name__
    if to_pdf:
        report_pdf(f'{name} {type(f).__name__} Complexity Report', report, f'{name}_complexity.pdf')
    else:
        print(report)
