...
def report_pdf(report_name, report, filename):
    pdf = PDF()
    pdf.add_page()
...
def object_decorated(f, to_pdf):
    def ff(*args, **kwargs):
        ...
        if to_pdf:
            report_pdf('Object', report, name + '_object.pdf')
        else:
            print(report)
...
def report_object(f):
    return object_decorated(f, True)

def stat_object(f):
    return object_decorated(f, False)
...
def stat_complexity(f, to_pdf=False):
    ...
    if to_pdf:
        report_pdf('Complexity', report, f.__name__ + '_complexity.pdf')
    else:
        print(report)
...
def report_complexity(f):
    return stat_complexity(f, to_pdf=True)
